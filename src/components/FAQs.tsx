...
const AccordionItem = ({
  ...
  const handleToggle = () => {
    const now = Date.now();
    if (now - lastToggleTime.current < 600) return; // throttle: 400ms
    lastToggleTime.current = now;
    setIsOpen((prev) => !prev);
  };

  return (
    <div className="py-7 border-b border-white/30" onClick={handleToggle}>
  ...
  return (
    <div className="bg-black text-white bg-gradient-to-b from-[#5D2CA8] to-black py-[72px] sm:py-24">
      <div className="container">
        <h1 className="text-center tracking-tighter sm:text-6xl sm:max-w-[648px] text-5xl font-bold mx-auto">
